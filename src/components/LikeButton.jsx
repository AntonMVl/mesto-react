...
export function LikeButton({card}){
    ...
    function handleLike() {
        if (isLike) {
            ...
            .catch((err)=> console.error(`Ошибка при сбросе лайка ${err}`))
        } else {
    ...
    return (
        <>
            <button type="button" className='content__link' onClick={handleLike}>
                <img
                    className={`content__like-img ${isLike && 'content__like-img_type_active'}`}
                    src={likeImage}
                    alt="Лайк"
                />
            </button>
